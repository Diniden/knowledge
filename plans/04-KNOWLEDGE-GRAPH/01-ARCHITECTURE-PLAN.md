...
> **Q**: The plan splits each spec into three files (`spec.json`, `content.md`, `metadata.json`). Is this the right granularity, or should `spec.json` and `metadata.json` be merged into a single file to reduce filesystem operations?
...
> **Q**: Should the `content.md` file support frontmatter (YAML header) for any per-content metadata, or should all metadata remain strictly in `metadata.json`?
...
> **Q**: Is there a performance concern with thousands of small directories in `specs/`? Should we benchmark directory listing performance at 10K, 50K, and 100K spec directories?
...
> **Q**: At what spec count should sharding activate? Is 10,000 the right threshold, or should it be lower (1,000) or higher (100,000)?
...
> **Q**: Should sharding be based on the first N characters of the ID, or use a hash-based distribution?
...
> **Q**: If sharding is enabled, should the shard depth be configurable (1 level vs. 2 levels of prefix directories)?
...
> **Q**: Should edges be stored as individual files (one per edge) or grouped by source spec (all edges from spec X in one file)?
...
> **Q**: For bidirectional edge types (`related-to`, `contradicts`), should there be one file stored under the lexicographically smaller spec ID, or always under the source?
...
> **Q**: Should edge files be co-located with specs (`specs/{id}/edges/`) rather than in a separate `edges/` directory?
...
> **Q**: Should the project use nanoid (21 chars, URL-safe) or UUID v7 (sortable by time, widely recognized)?
...
> **Q**: Is the `sp_`, `eg_`, `dc_`, `iq_` prefix convention sufficient for type identification, or should prefixes be longer/more descriptive?
...
> **Q**: Should IDs be case-sensitive? The plan uses `A-Za-z0-9_-` which is case-sensitive, but some systems (Windows filesystem) are case-insensitive.
...
> **Q**: Should deleted entity IDs be tombstoned (stored in a deletion log) to prevent accidental reuse, or is nanoid's entropy sufficient?
...
> **Q**: Should spec IDs be exposed directly in URLs and user-facing UI, or should there be a human-readable slug alongside the technical ID?
...
> **Q**: If a spec's title changes, should any URL slug change? Or should slugs be immutable once assigned?
...
> **Q**: Should the spec `status` field include a `review` state between `draft` and `active`?
...
> **Q**: Should specs have a `priority` or `importance` field that agents can use for traversal weighting?
...
> **Q**: Should the `createdAt` and `updatedAt` fields be stored in the JSON or derived entirely from git commit timestamps?
...
> **Q**: Should specs support a `parentSpecId` field for hierarchical decomposition, or should hierarchy be expressed purely through edges?
...
> **Q**: Should the embedding vector be stored in `metadata.json` or in a separate file to keep metadata small?
...
> **Q**: Should the `custom` field in metadata have any structure at all, or be fully free-form?
...
> **Q**: Should metadata include a `quality` score that agents compute to indicate spec completeness, clarity, and usefulness?
...
> **Q**: Should the `confidence` field be required or optional? If optional, what default should be assumed?
...
> **Q**: Should the edge `createdBy` field distinguish between human-created and agent-created edges?
...
> **Q**: Should documents support nested sections or sub-documents, or is a flat ordered list of specs sufficient?
...
> **Q**: Should documents have their own permissions separate from their constituent specs, or are document permissions derived from spec permissions?
...
> **Q**: Should documents support a `template` concept where new documents can inherit structure from a template?
...
> **Q**: Should indexes be stored as JSON files in the repository (git-tracked) or generated at runtime and stored outside git (ephemeral)?
...
> **Q**: Should indexes be considered a cache (rebuildable from source data) or authoritative (consulted as the source of truth)?
...
> **A**: In-memory indexes are viable up to ~100K specs. A full adjacency index for 100K specs with 5 edges each is ~50K entries × ~200 bytes ≈ 10MB in memory. A tag index, status index, and ID-to-path lookup add another ~20MB. Total ~30MB — trivial for a server process. No database fallback needed within the target scale.

> **Q**: Should index updates be synchronous (update index in the same operation as entity write) or asynchronous?
...
> **Q**: Should indexes include a sequence number or change counter so clients can detect if their cached version is current?
...
> **Q**: Should the edge index include full adjacency lists, or just edge IDs that require a second lookup?
...
> **Q**: Should there be a `status-index.json` mapping spec statuses to spec IDs for fast filtering?
...
> **Q**: Should there be a `search-index.json` for full-text search, or should full-text search always go through RAG?
> **A**: Full-text search goes through RAG (pgvector + metadata filtering). No separate full-text search index. The RAG layer handles both semantic similarity and keyword-style queries. Adding a parallel search index creates two search paths that can return inconsistent results. RAG is the single search interface.

> **Q**: Should there be a `dependency-graph-index.json` that pre-computes transitive closures for `depends-on` edges?
...
> **Q**: Should integrity constraints be enforced strictly (reject invalid operations) or permissively (allow and flag for later cleanup)?
...
> **Q**: Should integrity checks run on every write operation, or only periodically?
...
> **Q**: How should the system handle integrity violations discovered during git merge?
...
> **Q**: When a spec is deleted, should its edges be automatically deleted (cascade) or orphaned (flagged)?
...
> **Q**: When a spec is archived, should its edges be retained, archived, or weakened?
...
> **Q**: Should document-spec references be updated automatically when a spec is deleted, or should the document retain a "missing spec" placeholder?
...
> **Q**: Should migrations run automatically on server startup (if pending), or require explicit manual/CLI invocation?
...
> **Q**: Should migrations be forward-only (no rollback support), or must every migration have a `down()` function?
...
> **Q**: How should migrations handle multi-user scenarios where one user's client is ahead of another's schema version?
...
> **Q**: Should migrations also handle index schema changes, or are indexes always rebuilt from scratch when their schema changes?
...
> **Q**: Should migrations be testable in isolation (unit tests for each migration script)?
...
> **Q**: How long should a migration be expected to take for 10K specs? 100K specs?
...
> **Q**: What is the target maximum number of edges?
> **A**: Target: 250,000 edges (5 edges per spec at 50K specs). The in-memory adjacency index at 250K edges ≈ 50MB, well within server memory. Hot specs (heavily connected) may have 20–50 edges; most specs will have 2–5.

> **Q**: What is the target maximum knowledge graph size on disk?
> **A**: Target: ~500MB for 10K specs, ~2.5GB for 50K specs. This is acceptable for git. Embedding vectors are NOT in the repo (they're in pgvector), so the per-spec footprint is closer to 5–20KB. At 10KB average per spec, 50K specs = ~500MB. Perfectly manageable.

> **Q**: What is the target latency for reading a single spec?
...
> **Q**: Should frequently accessed specs be cached in memory, or is filesystem access fast enough with SSD?
> **A**: Implement an LRU in-memory cache for the 1,000 most recently accessed specs. SSD reads are fast but still involve syscalls. A memory cache eliminates filesystem overhead for hot specs. Cache invalidation: evict on write to that spec.

> **Q**: Should the storage layer implement read-through caching?
...
> **Q**: What is the target latency for writing a single spec update?
...
> **Q**: Should writes be buffered and batched, or written immediately?
...
> **Q**: Should index updates be deferred to a background process to keep write latency low?
...
> **Q**: In a multi-user scenario with a shared git remote, how often should the server pull and push?
...
> **Q**: Should file-level locking use OS-level file locks, advisory locks in a database, or a dedicated lock service?
...
> **Q**: Should the `inquiries/` directory be git-tracked, or stored only in the database?
...
> **Q**: How does git performance degrade with many small files? At 100K spec directories (300K files), are git operations still fast?
> **A**: With the hash-bucketed layout, 100K specs = ~200K files across 256 buckets plus ~100K edge files across 256 edge buckets. Git handles this via `core.fsmonitor` (FSMonitor) and sparse index features. `git status` at 300K files takes 1–3 seconds without FSMonitor, <500ms with it. Enable FSMonitor by default. `simple-git` supports all standard git operations efficiently.

> **Q**: Should the knowledge graph use git's sparse checkout feature to avoid loading the entire graph?
> **A**: No. Sparse checkout adds configuration complexity and breaks index rebuilds (which need all files). The full repo should always be checked out. Git's packfile format compresses similar files efficiently. If clone time becomes an issue at very large scale, shallow clones (`--depth 1`) are a better optimization than sparse checkout.

> **Q**: Should large embedding vectors be stored via Git LFS instead of in regular JSON files?
